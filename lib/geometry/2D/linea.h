...
namespace _2D
{
  ...
  public:
    ...
    Punto<T> fin;
    Linea();
    Linea(const Punto<T> &a, const Punto<T> &b);
    Linea(const Punto<T> &ini, T r, T a);
    T angulo() const;
    Linea rotar(T ang) const;
    T longitud() const;
    T longitud2() const;
    ...
    {
      return (ccw(inicio, fin, b.inicio) * ccw(inicio, fin, b.fin) <= 0) && (ccw(b.inicio, b.fin, inicio) * ccw(b.inicio, b.fin, fin) <= 0);
    }
  ...
  template <class T>
  Linea<T>::Linea(const Punto<T> &a, const Punto<T> &b) : inicio(a), fin(b)
  {
  }

  template <class T>
  Linea<T>::Linea(const Punto<T> &ini, T r, T a) : inicio(ini)
  {
